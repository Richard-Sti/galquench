...
def offsetPath(basePath, snapNum):
    ...
    offsetPath = basePath
    offsetPath += '/../postprocessing/offsets/offsets_%03d.hdf5' % snapNum
...
def loadObjects(basePath, snapNum, gName, nName, fields):
    ...
    with h5py.File(gcPath(basePath, snapNum), 'r') as f:
        ...
        if not result['count']:
            print('warn: 0 groups, empty return (snap=' + str(snapNum) + ').')
            return result
        ...
        for field in fields:
            ...
            if field not in f[gName].keys():
                raise Exception("Group catalog does not have "
    ...
    for i in range(header['NumFiles']):
        ...
        for field in fields:
            if field not in f[gName].keys():
                raise Exception("Group catalog does not have requested "
                                "field [" + field + "]!")
            ...
            if len(shape) == 1:
                result[field][wOff:wOff+shape[0]] = f[gName][field][0:shape[0]]
            else:
                rs = f[gName][field][0:shape[0], :]
                result[field][wOff:wOff+shape[0], :] = rs
...
def loadSingle(basePath, snapNum, haloID=-1, subhaloID=-1):
    ...
    if (haloID < 0 and subhaloID < 0) or (haloID >= 0 and subhaloID >= 0):
        raise Exception("Must specify either haloID or "
                        "subhaloID (and not both).")
